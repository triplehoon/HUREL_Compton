    ...
    public partial class CRUXELLLACC  //only cruxell function to make class for Compton GUI
    {
        ...
        private void Ppx_SelectedIndexChanged()
        {
            ...
            if (len > maxLen)
            {
                ...
                {
                    Trace.WriteLine("Please correct MaxPacketSize in Descriptor", "Invalid MaxPacketSize");
                    return;
                }
                ppx = maxLen / (EndPoint.MaxPktSize);
                ppx -= (ppx % 8);
                Trace.WriteLine("Maximum of 4MB per transfer.  Packets reduced.", "Invalid Packets per Xfer.");
            ...
            {
                PPX = 8;
                Trace.WriteLine("Minimum of 8 Packets per Xfer required for HS/SS Isoc.", "Invalid Packets per Xfer.");
            }
            if ((SelectedDevice.CyDevice.bHighSpeed) && (EndPoint.Attributes == 1))
            {
                ...
                {
                    PPX = 128;
                    Trace.WriteLine("Maximum 128 packets per transfer for High Speed Isoc", "Invalid Packets per Xfer.");
                }
            }
        ...
        private void init_file_save_bin()
        {
            ...
            else // if exits
            {
                ...
                while (true)
                {
                    string new_file_path = System.Windows.Forms.Application.StartupPath + "\\" + currentTime + Variables.FileName + $"_{file_name_counter}";
